...
int main(int argc, char* argv[])
{
    ...
    {
        if(first<=finished_count&&finished_count<=last)
		{
            for(i=finished_count;i<=last;i++)
            {
                finish=false;
                for(j=col_e-1;j>=0&&finish==false;j--)
                {
                ...
                if(i==finished_count&&myid!=size-1){
                    MPI_Send(&ed[i][0], eliminated, MPI_INT, myid+1, finished_count+1, MPI_COMM_WORLD);
                }
                if(i==finished_count&&now_index!=-1)
                {
                    for(k=0;k<col_e;k++)
                        er[now_index][k]=ed[finished_count][k];
                    now_index=-1;
        ...
        }else if(finished_count<first)
		{
            MPI_Recv(&ed[finished_count][0],eliminated, MPI_INT,myid-1,finished_count+1,MPI_COMM_WORLD,&status);
			if(myid!=size-1)
				MPI_Send(&ed[finished_count][0], eliminated, MPI_INT, myid+1, finished_count+1, MPI_COMM_WORLD);
    ...
    if(myid!=0)
		MPI_Send(&ed[first][0],(last-first+1)*eliminated,MPI_INT,0,myid,MPI_COMM_WORLD);
	else{
		for(int id=1;id<size;id++)
		{
			int f,l;
			if (id < eliminated % size)
        	{
                f = id * (my_n + 1);
                l = f + my_n;
        	}
        	...
        	{
                f = eliminated % size * (my_n + 1) + (id - eliminated % size) * my_n;
                l = f + my_n - 1;
        	}
			MPI_Recv(&ed[f][0],eliminated*(l-f+1),MPI_INT,id,id,MPI_COMM_WORLD,&status);
		}
    }
    MPI_Finalize();		
    // end time
